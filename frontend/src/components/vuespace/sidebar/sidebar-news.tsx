...
export function News({ articles }: { articles: NewsArticle[] }) {
    ...
    return cards.length || showCompleted ? (
        <div
            className="group/news overflow-hidden px-3 pb-3 pt-8"
                    ...
                    .map(({ href, title, summary, image }, idx) => (
                        <div
                            ...
                            className={cn(
                                ...
                                cardCount - idx > 3
                                    ? [
                                          "opacity-0 sm:group-hover/news:translate-y-[var(--y)] sm:group-hover/news:opacity-[var(--opacity)]",
                                          "sm:group-has-[*[data-dragging=true]]/news:translate-y-[var(--y)] sm:group-has-[*[data-dragging=true]]/news:opacity-[var(--opacity)]",
                                      ]
                            ...
                            style={
                                {
                                    "--y": `-${
                                        (cardCount - (idx + 1)) * OFFSET_FACTOR
                                    }%`,
                                    "--scale":
                                        1 -
                                        (cardCount - (idx + 1)) * SCALE_FACTOR,
                                    "--opacity":
                                        cardCount - (idx + 1) >= 6
                                            ? 0
                                            : 1 -
...
function NewsCard({
    ...
    const onPointerDown = (e: React.PointerEvent) => {
        ...
        ref.current.style.setProperty(
            "--w",
            ref.current.clientWidth.toString()
        );
    };
    ...
    const onClick = () => {
        ...
        if (
            ...
            (!drag.current.startTime ||
                Date.now() - drag.current.startTime < 250)
        ) {
            ...
            <div className={cn(hideContent && "invisible")}>
                ...
                <div
                    className={cn(
                        ...
                        "sm:group-has-[*[data-dragging=true]]/news:h-7 sm:group-has-[*[data-dragging=true]]/news:opacity-100 sm:group-hover/news:group-data-[active=true]/news:h-7 sm:group-hover/news:group-data-[active=true]/news:opacity-100"
                    )}
                >
